...
/**
 * <pre>
 * SimpleDateFormat result = new SimpleDateFormat(DATE_FORMAT_STR_ISO8601_Z);
 * result.setTimeZone(TimeZone.getTimeZone("UTC"));
 * objectMapper.setDateFormat(DATE_FORMAT_ISO8601_Z);
 * </pre>
 * 
...
public class StdDateFormatCopyFromJackson
                                          extends DateFormat {
    /*
    ...
    /**
     * For error message we'll also need a list of all formats.
     */
    ...
    /**
     * By default we use UTC for everything, with Jacksons 2.7 and later (2.6 and
     * earlier relied on GMT)
     */
     ...
     * sorts.
     */
    public final static StdDateFormatCopyFromJackson instance = new StdDateFormatCopyFromJackson();
     ...
     * <p>
     * Can not be `final` because {@link #setLenient(boolean)} returns `void`.
     *
     * @since 2.7
     */
    ...
    public StdDateFormatCopyFromJackson() {
        _locale = DEFAULT_LOCALE;
    }

    @Deprecated // since 2.7
    public StdDateFormatCopyFromJackson(TimeZone tz, Locale loc) {
        _timezone = tz;
        _locale = loc;
    }

    protected StdDateFormatCopyFromJackson(TimeZone tz, Locale loc, Boolean lenient) {
        _timezone = tz;
        _locale = loc;
        _lenient = lenient;
    }
     ...
     */
    public StdDateFormatCopyFromJackson withTimeZone(TimeZone tz) {
        if (tz == null) {
            tz = DEFAULT_TIMEZONE;
        }
        ...
        return new StdDateFormatCopyFromJackson(tz, _locale, _lenient);
    }

    public StdDateFormatCopyFromJackson withLocale(Locale loc) {
        if (loc.equals(_locale)) {
            return this;
        }
        return new StdDateFormatCopyFromJackson(_timezone, loc, _lenient);
    }

    @Override
    public StdDateFormatCopyFromJackson clone() {
        /*
         * Although there is that much state to share, we do need to orchestrate
         * a bit, mostly since timezones may be changed
         */
        return new StdDateFormatCopyFromJackson(_timezone, _locale, _lenient);
    }
    ...
    /**
     * Need to override since we need to keep track of leniency locally, and not
     * via underlying {@link Calendar} instance like base class does.
     */
    @Override // since 2.7
    ...
    public Date parse(String dateStr) throws ParseException {
        ...
        if (looksLikeISO8601(dateStr)) { // also includes "plain"
            ...
            while (--i >= 0) {
                ...
                if (ch < '0' || ch > '9') {
                    // 07-Aug-2013, tatu: And [databind#267] points out that
            ...
            if ((i < 0)
                // let's just assume negative numbers are fine (can't be
                // RFC-1123 anyway); check length for positive
    ...
    public Date parse(String dateStr, ParsePosition pos) {
        if (looksLikeISO8601(dateStr)) { // also includes "plain"
            ...
            } catch (ParseException e) { // will NOT be thrown due to false but
                                         // is declared...
                return null;
            }
        }
    ...
    public StringBuffer format(Date date, StringBuffer toAppendTo,
        ...
        if (_formatISO8601 == null) {
            _formatISO8601 = _cloneFormat(DATE_FORMAT_ISO8601_Z, DATE_FORMAT_STR_ISO8601_Z,
    ...
    protected Date parseAsISO8601(String dateStr, ParsePosition pos, boolean throwErrors)
        ...
        /*
         * First: do we have "zulu" format ('Z' == "UTC")? If yes, that's quite
         * simple because we already set date format timezone to be UTC, and
